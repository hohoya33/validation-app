...
class ErrorList extends Component {
    ...
    moveToLine (firstLine, firstColumn, lastLine, lastColumn) {
        const aceEditor = this.aceEditor.value;
        console.log(aceEditor);

        //this.props.onErrorSelect();
    ...
    render() {
        function ValidationAlert({ errorCount }) {
            if (errorCount) {
                return <div className="alert alert-danger" role="alert">Oops, I found {errorCount} error.</div>;
            }

            return <div className="alert alert-success" role="alert">Good job! No errors found.</div>;
        }



        const errorCount = this.props.messages.length;
        const messgeList = this.props.messages.map((msg, index) => {
            console.log(msg)

            let range = '';
            let lineText = '';
            let { type, message, extract, firstColumn, firstLine, hiliteLength, hiliteStart, lastColumn, lastLine } = msg;
            ...
            if (type === 'error') {
                return (
                    <li className="list-group-item" key={index}>
                        <a href="#source-code" onClick={() => this.moveToLine(firstLine, firstColumn, lastLine, lastColumn)}>
                            {lineText} :
                    </a>

                        <span>{message}</span>
                        <div id={`error-${index + 1}`}>{extract}</div>
                    </li>
                );
            }
        });
        ...
        return (
            <div className="col-md-8">
                ...
                <ValidationAlert errorCount={errorCount} />

                <div className="card">

                    <div className="card-header alert-warning">
                        Errors <a href="#" rel="tooltip" className="glyphicon glyphicon-question-sign" data-toggle="tooltip" data-placement="auto left" title="Click the link next to an error description to highlight the relevant lines in the Source window below"></a>
                    </div>
                ...
                <div className="card">
                    <div className="card-header alert-primary">Source</div>
                    <AceEditor
                        ref={this.aceEditor}
                        name='source-code'
                        className='card-body'
